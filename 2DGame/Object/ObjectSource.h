...
namespace gp
{
	namespace object
	{
		...
		{
		public:
			ObjectSource(std::string name, std::string path,float health, float damageCollision, sf::Vector2f relativeBoundingBoxSize = sf::Vector2f(1.f,1.f));
			~ObjectSource();

			std::vector<sf::Vector2f> m_boundingBoxPoints;

			float m_health;
			float m_damageCollision;
